...
int main() {
    ...
    printf("  │                                                     │\n");
    printf("  │  ✓  Always use platform_malloc for GM pointers      │\n");
    ...
    printf("  void* input = platform_malloc(size);\n");
    printf("  void* output = platform_malloc(size);\n\n");
    printf("  // 2. Copy input data to device\n");
    printf("  platform_memcpy_h2d(input, host_data, size);\n\n");
    printf("  // 3. Pack arguments\n");
    printf("  SimpleArgs args = {input, output, count, 0};\n\n");
    printf("  // 4. Launch kernel\n");
    printf("  platform_kernel_launch(kernel, blocks, &args, sizeof(args), stream);\n\n");
    printf("  // 5. Synchronize\n");
    printf("  platform_stream_sync(stream);\n\n");
    printf("  // 6. Copy output back\n");
    printf("  platform_memcpy_d2h(host_result, output, size);\n\n");

    /* Cleanup */
    printf("Step 8: Cleanup...\n");
    platform_free(input);
    platform_free(output);
    platform_shutdown();
    printf("  Done\n");
