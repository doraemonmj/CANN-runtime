/**
 * 04-aicpu-kernel-launch - AICPU Kernel Launch with Backend Server Pattern
 *
...
struct DeviceArgs {
    ...
    uint64_t customArgsPtr{0};     // HBM pointer to AddArgs (our workaround)
};
...
int main() {
    ...
    AicpuSoInfo soInfo{};
    rc = soInfo.Init("./kernel/libadd_aicpu_kernel.so");
    if (rc != 0) {
     ...
     * Hardware argument passing: Our AddArgs must live in HBM for AICPU
     * to access. We pass pointer to it through DeviceArgs.customArgsPtr. */
    AddArgs* dev_add_args = nullptr;
    rc = rtMalloc(reinterpret_cast<void**>(&dev_add_args), sizeof(AddArgs),
                  RT_MEMORY_HBM, 0);
    if (rc != 0) {
        std::cerr << "rtMalloc for AddArgs failed: " << rc << '\n';
        soInfo.Finalize();
        rtFree(dev_output);
        rtFree(dev_input_a);
        rtFree(dev_input_b);
    ...
    AddArgs add_args;
    add_args.input_a = dev_input_a;
    add_args.input_b = dev_input_b;
    add_args.output = dev_output;
    add_args.count = N;

    /* Hardware transfer: Copy AddArgs to HBM */
    rc = rtMemcpy(dev_add_args, sizeof(AddArgs), &add_args,
                  sizeof(AddArgs), RT_MEMCPY_HOST_TO_DEVICE);
    if (rc != 0) {
        std::cerr << "rtMemcpy AddArgs failed: " << rc << '\n';
        rtFree(dev_add_args);
        soInfo.Finalize();
        rtFree(dev_output);
        rtFree(dev_input_a);
    ...
    deviceArgs.customArgsPtr = reinterpret_cast<uint64_t>(dev_add_args);
